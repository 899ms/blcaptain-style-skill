...
| 维度 | 内容 |
|---|---|
| **版式** | 3 套视觉语言 · **35 种版式当前在用**（静纸 18 / 实证 12 / 图桥 5），三语言引擎共 60+ 版式渲染器；每种还有构图重心 · 纸色 · 装置三轴变体 |
| **主题** | 11 套经过验证的主题色板（不让你乱填 hex，乱填只会变丑） |
| **图源** | 国内 + 海外 14+ 公开图库、4 档版权分级；**国内 CC0 源前置、开箱即用不翻墙**，Unsplash / Pexels 等国际源高质量补充 |
...
**7 封面母体 + 11 正文骨架**（随笔 / 清单 / 图集 / 边注时间轴 / 手绘路线图 / 引言 / 账目 / 矩阵 / 对照 / 流程 / 核心词…），另含**雾野 Mist Field 母版系列 13 种**（R01–R08 封面母体 + 正文骨架，7 张 sha256 锁定 accepted 母版）。

### 实证 · Signal Proof（证据）
把内容校验成可信的证据——**界面是证据层，读者才相信流程**。奶白档案纸 + 电蓝 + VERIFIED 印章。
**6 种封面**（纯文字主张 / 趋势数据 / 真截图 + hero / 宣言 / 图＋数据 split）+ **6 种证据卡**（截图舞台 / 洞察 / 数据 / 工作流 / 双栏对照 / 田野笔记）。
...
| 物件 / 自然现场 | 静纸 | 物件特写 + 图集 |
| 情感 / 治愈 / 夜思 | 静纸（夜纹暗调）| 引言封面 + 随笔 + 引言 |
| 美食 / 餐桌 / 生活方式 | 静纸（炉台暖调）| 质感封面 + 随笔 + 引言 |
| 影视 / 观影 / 日常记录 | 静纸 | 标题封面 + 随笔 + 引言 |
| 家居 / 出租屋改造 | 静纸 | 标题封面 + 清单 + 随笔 |
...
2. **公开免费图库**——你没有图时，从**国内 + 海外 14+ 公开源**里找：**国内 CC0 源（cc0.cn / 泼辣有图 / hippopx / 别样网…）前置、不用翻墙**，**海外 Unsplash / Pexels / StockSnap 等作高质量补充**；按 4 档版权分级，CC0 / 公共领域优先。
...
├── references/         # 按需加载：图源工作流 / 版式契约 / 主题色板 / 最佳实践
...
**Q：没有图怎么办？**
A：优先用你自己的照片 / 截图；没有就从免费图库找（国内 CC0 源前置开箱即用、海外 Unsplash/Pexels 补充），会记录来源；都没有才克制地用 AI 生图。
